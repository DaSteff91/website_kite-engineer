import Image from "next/image";
import background_image_darker from "@/public/images/background_image_darker.jpeg";
import engineer_hero_image from "@/public/images/engineer_hero_image.jpg";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { ArrowRight, ArrowLeft, MessageSquareCode, FileSearch, Microscope, GraduationCap, UserRoundCheck, TrendingUp, BookOpen, Lightbulb } from "lucide-react";

export default function TechnicalConsultingPage() {
  return (
    <main className="relative min-h-screen">
      {/* Background Image*/}
      <div className="fixed inset-0 -z-10">
        <Image
          src={background_image_darker}
          alt="Background"
          fill
          className="brightness-50 object-cover object-top"
          priority
          placeholder="blur"
        />
      </div>

      {/* Hero Section */}
      <section className="relative h-[50vh] sm:h-[60vh] flex items-center justify-center">
        <div className="absolute inset-0">
          <Image
            src={engineer_hero_image}
            alt="Technical Consulting"
            fill
            className="object-cover object-[center_20%] brightness-50"
            priority
            placeholder="blur"
          />
        </div>
        <div className="relative z-10 container mx-auto px-4 text-center">
          <h1
            className="pb-1 text-3xl sm:text-4xl md:text-5xl font-bold mb-4 sm:mb-6 
   bg-gradient-to-r from-white via-gray-200 to-gray-300 
   bg-clip-text text-transparent 
   [text-shadow:0_0_8px_rgba(209,213,219,0.6)]"
          >
            Technical Consulting Services
          </h1>
        </div>
      </section>

      {/* Content Section */}
      <section className="py-12 sm:py-16">
        <div className="container mx-auto px-4">
          {/* Back Navigation */}
          <div className="mb-8">
            <Link 
              href="/engineer" 
              className="inline-flex items-center gap-2 text-blue-400 hover:text-blue-300 transition-colors"
            >
              <ArrowLeft className="h-4 w-4" />
              Back to Engineering Services
            </Link>
          </div>

          <div className="relative mb-16 sm:mb-20">
            <h2
              className="text-xl sm:text-2xl md:text-3xl font-bold text-center max-w-4xl mx-auto leading-relaxed 
   bg-[linear-gradient(to_right,white,rgba(229,231,235,0.8),rgba(209,213,219,1))] 
   bg-clip-text text-transparent animate-gradient"
            >
              Expert Technical Guidance for Strategic Decisions
            </h2>
            <p className="mt-4 text-lg sm:text-xl text-center text-white/80 max-w-3xl mx-auto leading-relaxed">
              Independent technical expertise to help you navigate complex challenges and opportunities
            </p>
            <div className="absolute -bottom-6 sm:-bottom-8 left-1/2 transform -translate-x-1/2 w-20 sm:w-24 h-1 bg-gradient-to-r from-blue-500 via-teal-500 to-emerald-500 rounded-full"></div>
          </div>

          {/* Services Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto mb-16">
            <div className="bg-gradient-to-br from-blue-900/20 via-card/20 to-cyan-900/20 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] hover:shadow-[0_8px_32px_rgba(255,255,255,0.1)] transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                <FileSearch className="h-6 w-6 text-blue-400" />
                <h3 className="text-lg font-semibold text-blue-200">Process Assessment</h3>
              </div>
              <ul className="text-white/80 space-y-2 text-sm">
                <li>• Independent process evaluation</li>
                <li>• Bottleneck identification and analysis</li>
                <li>• Improvement opportunity assessment</li>
                <li>• Best practice benchmarking</li>
                <li>• Objective third-party perspective</li>
              </ul>
            </div>

            <div className="bg-gradient-to-br from-cyan-900/25 via-card/25 to-teal-900/25 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] hover:shadow-[0_8px_32px_rgba(255,255,255,0.1)] transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                <Microscope className="h-6 w-6 text-cyan-400" />
                <h3 className="text-lg font-semibold text-cyan-200">Technical Research</h3>
              </div>
              <ul className="text-white/80 space-y-2 text-sm">
                <li>• Technology landscape analysis</li>
                <li>• Literature review and synthesis</li>
                <li>• Patent and IP research</li>
                <li>• Market trend identification</li>
                <li>• Technical feasibility studies</li>
              </ul>
            </div>

            <div className="bg-gradient-to-br from-teal-900/30 via-card/30 to-emerald-900/30 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] hover:shadow-[0_8px_32px_rgba(255,255,255,0.1)] transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                <GraduationCap className="h-6 w-6 text-teal-400" />
                <h3 className="text-lg font-semibold text-teal-200">Training & Knowledge Transfer</h3>
              </div>
              <ul className="text-white/80 space-y-2 text-sm">
                <li>• Technical training programs</li>
                <li>• Knowledge sharing workshops</li>
                <li>• Best practice documentation</li>
                <li>• Skill development planning</li>
                <li>• Mentoring and coaching</li>
              </ul>
            </div>

            <div className="bg-gradient-to-br from-emerald-900/35 via-card/35 to-blue-900/35 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] hover:shadow-[0_8px_32px_rgba(255,255,255,0.1)] transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                <UserRoundCheck className="h-6 w-6 text-emerald-400" />
                <h3 className="text-lg font-semibold text-emerald-200">Competitive Intelligence</h3>
              </div>
              <ul className="text-white/80 space-y-2 text-sm">
                <li>• Competitor technology analysis</li>
                <li>• Market positioning assessment</li>
                <li>• Strategic intelligence gathering</li>
                <li>• Competitive advantage identification</li>
                <li>• Regular monitoring and reporting</li>
              </ul>
            </div>

            <div className="bg-gradient-to-br from-blue-900/40 via-card/40 to-cyan-900/40 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] hover:shadow-[0_8px_32px_rgba(255,255,255,0.1)] transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                <TrendingUp className="h-6 w-6 text-blue-400" />
                <h3 className="text-lg font-semibold text-blue-200">Strategic Planning</h3>
              </div>
              <ul className="text-white/80 space-y-2 text-sm">
                <li>• Technology roadmap development</li>
                <li>• Investment prioritization</li>
                <li>• Risk assessment and mitigation</li>
                <li>• Future scenario planning</li>
                <li>• Decision support analysis</li>
              </ul>
            </div>

            <div className="bg-gradient-to-br from-cyan-900/45 via-card/45 to-teal-900/45 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] hover:shadow-[0_8px_32px_rgba(255,255,255,0.1)] transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                <Lightbulb className="h-6 w-6 text-cyan-400" />
                <h3 className="text-lg font-semibold text-cyan-200">Innovation Consulting</h3>
              </div>
              <ul className="text-white/80 space-y-2 text-sm">
                <li>• Innovation strategy development</li>
                <li>• Technology scouting and evaluation</li>
                <li>• R&D portfolio optimization</li>
                <li>• Innovation process improvement</li>
                <li>• Collaboration opportunity identification</li>
              </ul>
            </div>
          </div>

          {/* Consulting Approach */}
          <div className="bg-gradient-to-br from-blue-900/15 via-card/15 to-cyan-900/15 backdrop-blur-sm p-6 sm:p-8 rounded-lg border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] max-w-4xl mx-auto mb-16">
            <h3 className="text-xl sm:text-2xl font-bold mb-6 text-center text-blue-200">Our Consulting Approach</h3>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="text-center">
                <div className="bg-blue-500/20 rounded-full w-12 h-12 flex items-center justify-center mx-auto mb-4">
                  <span className="text-blue-300 font-bold text-lg">1</span>
                </div>
                <h4 className="font-semibold text-white mb-3">Discovery</h4>
                <p className="text-white/80 text-sm leading-relaxed">
                  Understanding your challenges, objectives, and current state through detailed analysis.
                </p>
              </div>
              <div className="text-center">
                <div className="bg-cyan-500/20 rounded-full w-12 h-12 flex items-center justify-center mx-auto mb-4">
                  <span className="text-cyan-300 font-bold text-lg">2</span>
                </div>
                <h4 className="font-semibold text-white mb-3">Research</h4>
                <p className="text-white/80 text-sm leading-relaxed">
                  Comprehensive research and analysis using industry expertise and technical knowledge.
                </p>
              </div>
              <div className="text-center">
                <div className="bg-teal-500/20 rounded-full w-12 h-12 flex items-center justify-center mx-auto mb-4">
                  <span className="text-teal-300 font-bold text-lg">3</span>
                </div>
                <h4 className="font-semibold text-white mb-3">Analysis</h4>
                <p className="text-white/80 text-sm leading-relaxed">
                  Objective evaluation and synthesis of findings with actionable recommendations.
                </p>
              </div>
              <div className="text-center">
                <div className="bg-emerald-500/20 rounded-full w-12 h-12 flex items-center justify-center mx-auto mb-4">
                  <span className="text-emerald-300 font-bold text-lg">4</span>
                </div>
                <h4 className="font-semibold text-white mb-3">Delivery</h4>
                <p className="text-white/80 text-sm leading-relaxed">
                  Clear, actionable insights and recommendations with implementation support.
                </p>
              </div>
            </div>
          </div>

          {/* Expertise Areas */}
          <div className="bg-gradient-to-br from-cyan-900/15 via-card/15 to-teal-900/15 backdrop-blur-sm p-6 sm:p-8 rounded-lg border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] max-w-4xl mx-auto mb-16">
            <h3 className="text-xl sm:text-2xl font-bold mb-6 text-center text-cyan-200">Technical Expertise Areas</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="font-semibold text-white mb-3">Semiconductor Technology</h4>
                <ul className="text-white/80 text-sm space-y-1">
                  <li>• Advanced process technologies</li>
                  <li>• Manufacturing equipment and methods</li>
                  <li>• Yield optimization strategies</li>
                  <li>• Quality control systems</li>
                  <li>• Industry trends and roadmaps</li>
                </ul>
              </div>
              <div>
                <h4 className="font-semibold text-white mb-3">Process Engineering</h4>
                <ul className="text-white/80 text-sm space-y-1">
                  <li>• Statistical process control</li>
                  <li>• Advanced process control (APC)</li>
                  <li>• Six Sigma and Lean methodologies</li>
                  <li>• Process optimization techniques</li>
                  <li>• Change management protocols</li>
                </ul>
              </div>
              <div>
                <h4 className="font-semibold text-white mb-3">Data Analytics & AI</h4>
                <ul className="text-white/80 text-sm space-y-1">
                  <li>• Machine learning applications</li>
                  <li>• Predictive analytics models</li>
                  <li>• Data visualization and dashboards</li>
                  <li>• Python-based analysis tools</li>
                  <li>• AI implementation strategies</li>
                </ul>
              </div>
              <div>
                <h4 className="font-semibold text-white mb-3">Software & Automation</h4>
                <ul className="text-white/80 text-sm space-y-1">
                  <li>• Custom software development</li>
                  <li>• Workflow automation solutions</li>
                  <li>• Database design and management</li>
                  <li>• System integration approaches</li>
                  <li>• Technology stack selection</li>
                </ul>
              </div>
            </div>
          </div>

          {/* Value Proposition */}
          <div className="bg-gradient-to-br from-teal-900/15 via-card/15 to-emerald-900/15 backdrop-blur-sm p-6 sm:p-8 rounded-lg border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] max-w-4xl mx-auto mb-16">
            <h3 className="text-xl sm:text-2xl font-bold mb-6 text-center text-teal-200">Why Choose Our Technical Consulting?</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="font-semibold text-white mb-3">Independent Perspective</h4>
                <p className="text-white/80 text-sm leading-relaxed">
                  Objective, unbiased analysis free from internal politics or vendor interests. 
                  Fresh insights that internal teams might miss due to proximity to the challenges.
                </p>
              </div>
              <div>
                <h4 className="font-semibold text-white mb-3">Deep Technical Knowledge</h4>
                <p className="text-white/80 text-sm leading-relaxed">
                  Extensive experience in semiconductor manufacturing, process engineering, 
                  and software development with practical, hands-on expertise.
                </p>
              </div>
              <div>
                <h4 className="font-semibold text-white mb-3">Rapid Knowledge Transfer</h4>
                <p className="text-white/80 text-sm leading-relaxed">
                  Efficient research and analysis capabilities that quickly synthesize complex 
                  technical information into actionable insights and recommendations.
                </p>
              </div>
              <div>
                <h4 className="font-semibold text-white mb-3">Cost-Effective Expertise</h4>
                <p className="text-white/80 text-sm leading-relaxed">
                  Access to specialized knowledge without the overhead of full-time experts. 
                  Flexible engagement models that scale with your needs and budget.
                </p>
              </div>
            </div>
          </div>

          {/* Contact Section */}
          <div className="text-center">
            <p className="text-xl text-white/90 mb-8 max-w-2xl mx-auto">
              Ready to gain strategic technical insights? Let's provide the expert guidance you need to make informed decisions.
            </p>
            <Link href="/contact?subject=Technical Consulting Services Inquiry&message=Hello Kite-Engineer,%0A%0AI'm interested in your technical consulting services. Please provide information about:%0A%0A- Process assessment and evaluation capabilities%0A- Technical research and analysis services%0A- Training and knowledge transfer options%0A- Competitive intelligence and strategic planning%0A%0AMy consulting needs:%0A[Please describe your current technical challenges, decision-making requirements, knowledge gaps, and strategic objectives]%0A%0ALooking forward to expert guidance!">
              <Button
                size="lg"
                className="group bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 transition-all duration-300 shadow-lg hover:shadow-xl"
              >
                Get Expert Guidance
                <ArrowRight className="ml-2 h-5 w-5 transition-transform group-hover:translate-x-1" />
              </Button>
            </Link>
          </div>
        </div>
      </section>
    </main>
  );
}